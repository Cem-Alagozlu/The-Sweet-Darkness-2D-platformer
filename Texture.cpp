...
void Texture::Draw(const Point2f& dstBottomLeft, const Rectf& srcRect, bool flipTexture) const
{
	if (!m_CreationOk)
	{
	...
	else
	{
		...
		Draw(vertexRect, srcRect, flipTexture);
	}
}

void Texture::Draw(const Rectf& destRect, const Rectf& srcRect, bool flipTexture) const
{
	if (!m_CreationOk)
	{
	...
	float textTop{};
	float textBottom{};
	if (!(srcRect.width > 0.0f && srcRect.height > 0.0f)) // No rect specified, use complete texture
	{
		textLeft = 0.0f;
		textRight = 1.0f;
		textBottom = 0.0f;
	...
	// Determine vertexCoordinates
	float vertexLeft{destRect.left};
	float vertexBottom{destRect.bottom};
	float vertexRight{};
	float vertexTop{};
	if (!(destRect.width > 0.0f && destRect.height > 0.0f)) // If no size specified use size of texture
	{
		vertexRight = vertexLeft + m_Width;
		vertexTop = vertexBottom + m_Height;
	}
	else
	{
	...
	// By default, textures are modulated with the current fragment's color
	...
	glEnable(GL_TEXTURE_2D);
	{
		glBegin(GL_QUADS);
		{
			if (!flipTexture)
			{
				// Map left-bottom texture -> left-top vertex
				glTexCoord2f(textLeft, textBottom);
				glVertex2f(vertexLeft, vertexTop);

				// left-top texture -> left-bottom vertex
				glTexCoord2f(textLeft, textTop);
				glVertex2f(vertexLeft, vertexBottom);

				// right-top texture -> right-bottom vertex
				glTexCoord2f(textRight, textTop);
				glVertex2f(vertexRight, vertexBottom);

				// Right-bottom texture -> right-top vertex
				glTexCoord2f(textRight, textBottom);
				glVertex2f(vertexRight, vertexTop);
			}
			else
			{
				// Map left-bottom texture -> right-top vertex
				glTexCoord2f(textLeft, textBottom);
				glVertex2f(vertexRight, vertexTop);

				// left-top texture -> right-bottom vertex
				glTexCoord2f(textLeft, textTop);
				glVertex2f(vertexRight, vertexBottom);

				// right-top texture -> left-bottom vertex
				glTexCoord2f(textRight, textTop);
				glVertex2f(vertexLeft, vertexBottom);

				// Right-bottom texture -> left-top vertex
				glTexCoord2f(textRight, textBottom);
				glVertex2f(vertexLeft, vertexTop);
			}
		}
...
void Texture::DrawFilledRect(const Point2f& dstBottomLeft) const
{
	glColor4f(1.0f, 0.0f, 1.0f, 1.0f);
	glBegin(GL_TRIANGLE_STRIP);
	{
		glVertex2f(dstBottomLeft.x, dstBottomLeft.y + m_Height);
		glVertex2f(dstBottomLeft.x, dstBottomLeft.y);
		glVertex2f(dstBottomLeft.x + m_Width, dstBottomLeft.y + m_Height);
		glVertex2f(dstBottomLeft.x + m_Width, dstBottomLeft.y);
	}
	glEnd();
}
